...
def extract_PS_Group_mismatches():
    print("Checking for PS Group Mismatches in Gold Layer...")
    ...
    mismatch_query = f"""
        SELECT 
            ...
            "Actual PS Group" AS "PA0001_PS_Group",
            "Planned PS Group" AS "ZPOST_PS_Group",
            
            -- Bring in every other column dynamically, excluding the ones explicitly handled
            * EXCLUDE("Personnel Number", "Actual PS Group", "Planned PS Group", {date_excludes_str})
        ...
        WHERE 
            ...
            AND "Actual PS Group" != "Planned PS Group"
    ...
    try:
        ...
        output_file = REPORTING_DIR / f"PSG_Mismatches_{current_date}.xlsx"
        ...
        print("Please verify the column names for ZPOST Planned PS Group in your Gold table.")
    finally:
        conn.close()

if __name__ == "__main__":
    extract_PS_Group_mismatches()
